  ...
  methods: {
    ...
    createUser(event) {
      ...
      if (this.messages.length > 0) {
        ...
        this.$http
          .post("https://api3-datame.herokuapp.com/api/v1/register", formData)
          .then(result => {
    ...
    login(event) {
      ...
      const baseURI = "https://api3-datame.herokuapp.com/api/v1/login";
      this.$http
        .post("https://api3-datame.herokuapp.com/api/v1/login", formLogin)
        .then(result => {
          this.$cookies.set("token", result.data.token);
          let token = `JWT ${this.$cookies.get("token")}`;
          this.$http
            .get("https://api3-datame.herokuapp.com/api/v1/whoami", {
              headers: { Authorization: token }
            })
            .then(result => {
